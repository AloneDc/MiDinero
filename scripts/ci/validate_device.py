...
class DeviceValidation(Validation):
    def __init__(self) -> None:
        ...
        self.result['states'] = {
            ...
            'DISTRIBUTABLE': 'NO_SIGNED_IPA', 'TESTFLIGHT_READY': 'NO_NOT_UPLOADED',
        }
    ...
    def validate_structure(self) -> None:
        ...
        actions = json.loads((metadata / 'extract.actionsdata').read_text())
        intent = actions['actions']['RegisterExpenseIntent']
        require(intent.get('isDiscoverable') is True, 'intent not discoverable in extracted metadata')
        require(intent.get('openAppWhenRun') is False, 'intent unexpectedly requests foreground')
        parameters = {item['name']: item for item in intent['parameters']}
        require(parameters['amount']['isOptional'] is False and parameters['category']['isOptional'] is False
                and parameters['note']['isOptional'] is True, 'intent parameter contract changed')
        shortcut = next(item for item in actions['autoShortcuts'] if item['actionIdentifier'] == 'RegisterExpenseIntent')
        require(len(shortcut['phraseTemplates']) == 2, 'missing shortcut phrases')
        require(all('${applicationName}' in item['key'] for item in shortcut['phraseTemplates']), 'missing app name in phrases')
